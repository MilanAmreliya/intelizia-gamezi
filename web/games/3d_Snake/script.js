...
(function(){ 'use strict';
   ...
   function handleSwipe() {
     ...
     if (Math.abs(dx) > Math.abs(dy)) {
       if (dx > SWIPE_THRESHOLD) g.currentState().keys.right = 1;
       else if (dx < -SWIPE_THRESHOLD) g.currentState().keys.left = 1;
     } else {
       if (dy > SWIPE_THRESHOLD) g.currentState().keys.down = 1;
       else if (dy < -SWIPE_THRESHOLD) g.currentState().keys.up = 1;
     }
   }
  /*================================================
