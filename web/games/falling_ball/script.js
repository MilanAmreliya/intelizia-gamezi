	...
	if (!gameStarted){
		...
		gameRunning = setInterval(function(){
			...
			for (var i = 0; i < currentBlocks.length; i++) {
                ...
                if (!iMovingBlock || !iMovingHole) continue;
				...
				if (iMovingBlockTop<-20) {
					...
					currentBlocks.shift();
					if (score>0) {}
					if (difficulty<5) { // max difficulty wil   l be 5 because condition will still run at difficulty 4 and make difficulty 5 when below conditions get fulfilled
